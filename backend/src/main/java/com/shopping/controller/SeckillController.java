...
public class SeckillController {
    ...
    public ApiResponse<SeckillResultResponse> getSeckillResult(@RequestParam Long userId, @RequestParam Long productId) {
        if (userId == null || userId <= 0 || productId == null || productId <= 0) {
            throw new BusinessException(ErrorCode.INVALID_REQUEST, "用户或商品参数不合法", HttpStatus.BAD_REQUEST);
        }

        SeckillResultResponse response = seckillService.getSeckillResult(userId, productId);
